...
const communityRewardsActions = [
  {
    icon: <Heart className="h-6 w-6 text-wasfah-deep-teal" />, // Changed from text-wasfah-coral-red
    label: "Health Tracking",
    path: "/health-tracking-home"
  },
  {
    icon: <Award className="h-6 w-6 text-wasfah-deep-teal" />, // Changed from text-wasfah-coral-red
    label: "Rewards",
    path: "/loyalty-program"
  },
  {
    icon: <Share2 className="h-6 w-6 text-wasfah-deep-teal" />, // Changed from text-wasfah-coral-red
    label: "Shared Recipes",
    path: "/shared-recipes"
  },
  {
    icon: <Users className="h-6 w-6 text-wasfah-deep-teal" />, // Changed from text-wasfah-coral-red
    label: "Community",
    path: "/community"
  },
  {
    icon: <Bot className="h-6 w-6 text-wasfah-deep-teal" />, // Changed from text-wasfah-coral-red
    label: "AI Chef",
    path: "/ai-chef"
  },
  {
    icon: <CreditCard className="h-6 w-6 text-wasfah-deep-teal" />, // Changed from text-wasfah-coral-red
    label: "Subscription",
    path: "/subscription"
  },
];
...
const NewHomePage: React.FC = () => {
  return (
    <div className="px-4">
      ...
      {/* Changed heading color to match the new icon color for consistency */}
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-3">
        {communityRewardsActions.map((action, index) => (
          <Link
            ...
            className="flex flex-col items-center p-3 sm:p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 hover:scale-105 text-center"
          >
            {/* Changed background color to match the new icon color */}
